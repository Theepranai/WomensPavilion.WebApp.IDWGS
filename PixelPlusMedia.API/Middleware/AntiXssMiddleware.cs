...
namespace PixelPlusMedia.API.Middleware
{
    public class AntiXssMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            // Check Xss in form files
            var isImageUpload = false;
            try
            {
                if (context.Request.Form.Files.Any())
                {
                    foreach (var form in context.Request.Form.Files)
                    {
                        if (!isImageUpload) isImageUpload = form.ContentType.Contains("image");

                        var fileName = form.FileName;
                        if (CrossSiteScriptingValidation.IsDangerousString(fileName, out _))
                        {
                            await RespondWithAnError(context).ConfigureAwait(false);
                            return;
                        }
        ...
        private Task RespondWithAnError(HttpContext context)
        {
            ...
            throw new Exception("XSS : Value incorrect format");
        }
    }
